...
envContent.split("\n").forEach((line) => {
  ...
  if (line && !line.startsWith("#")) {
    ...
    if (match) {
      ...
      // Set environment variable for all keys, not just Twilio ones
...
console.log(
  ...
  process.env.TWILIO_AUTH_TOKEN
    ? "***" + process.env.TWILIO_AUTH_TOKEN.slice(-4)
    : "NOT SET"
);
console.log("");
...
async function listNumbers() {
  try {
    ...
    numbers.forEach((number, i) => {
      ...
      console.log(
        `   Capabilities: Voice=${number.capabilities.voice}, SMS=${number.capabilities.sms}`
      );
